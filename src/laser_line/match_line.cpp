...
void LineMatcher::laserLinesCallback(const laser_line::Lines::ConstPtr &msg)
{
    laser_lines.clear();
...
void LineMatcher::mapLinesCallback(const laser_line::Lines::ConstPtr &msg)
{
    map_lines.clear();
    for (size_t i = 0; i < msg->lines.size(); i++)
    {
...
std::vector<Line> LineMatcher::selectLongestLines(const std::vector<Line> &lines, int n)
{
...
int LineMatcher::matchLines(const std::vector<Line> &selected_lines, const std::vector<Line> &map_lines,
...
double LineMatcher::calculateAngle(const Line &l)
{
...
double LineMatcher::normalLineAngle(const double angle)
{
    double radian = angle;
...
void LineMatcher::processOptimization()
{
...
visualization_msgs::Marker LineMatcher::lineToMarker(const Line &line, int id, const std::string &ns, const std::array<float, 4> &color, const std::string &frame_id)
{
...
void LineMatcher::publishLinesAsMarkers(const ros::Publisher &marker_pub, const std::vector<std::pair<Line, Line>> &matched_lines)
{
